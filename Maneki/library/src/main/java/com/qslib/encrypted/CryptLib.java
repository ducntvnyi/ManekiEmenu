...
public class CryptLib {
    ...
    public static String SHA256(String text, int length) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        System.out.println("SHA256:: " + resultStr);
        return resultStr;
    ...
    public static void main(String password, String plainText) {
        try {
            ...
            System.out.println("CryptLib:: encrypted text:: " + output);
            output = _crypt.decrypt(output, key, iv); //decrypt
            System.out.println("CryptLib:: decrypted text:: " + output);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private String encryptDecrypt(String _inputText, String _encryptionKey,
        ...
        int len = _encryptionKey.getBytes(UTF8).length; // length of the key	provided

        if (_encryptionKey.getBytes(UTF8).length > _key.length)
            len = _key.length;

        int ivlen = _initVector.getBytes(UTF8).length;

        if (_initVector.getBytes(UTF8).length > _iv.length)
            ivlen = _iv.length;

        System.arraycopy(_encryptionKey.getBytes(UTF8), 0, _key, 0, len);
        System.arraycopy(_initVector.getBytes(UTF8), 0, _iv, 0, ivlen);
        ...
        if (_mode.equals(EncryptMode.ENCRYPT)) {
            ...
            byte[] results = _cx.doFinal(_inputText.getBytes(UTF8)); // Finish
            // multi-part
            // transformation
            // (encryption)
