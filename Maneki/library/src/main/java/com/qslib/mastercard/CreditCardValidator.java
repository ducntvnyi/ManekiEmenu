...
public class CreditCardValidator {
    ...
    private CreditCard isAmericanExpress(String creditCardNumber) {
        try {
            if (creditCardNumber.length() == 15
                    && (creditCardNumber.startsWith("34") || creditCardNumber
                    .startsWith("37"))) {
    ...
    private CreditCard isChinaUnionPay(String creditCardNumber) {
        try {
            if ((creditCardNumber.length() >= 16 && creditCardNumber.length() <= 19)
                ...
                int firstDigits = Integer.parseInt(creditCardNumber.substring(
                        0, 6));
    ...
    private CreditCard isDinersClubUsAndCanada(String creditCardNumber) {
        try {
            if (creditCardNumber.length() == 16
                    && (creditCardNumber.startsWith("54") || creditCardNumber
                    .startsWith("55"))) {
    ...
    private CreditCard isDiscoverCard(String creditCardNumber) {
        try {
            if (creditCardNumber.length() == 16
                ...
                int firstThreeDigits = Integer.parseInt(creditCardNumber
                        .substring(0, 3));
                int firstSixDigits = Integer.parseInt(creditCardNumber
                        .substring(0, 6));
    ...
    private CreditCard isJCB(String creditCardNumber) {
        try {
            if (creditCardNumber.length() == 16) {
                int firstFourDigits = Integer.parseInt(creditCardNumber
                        .substring(0, 4));
    ...
    private CreditCard isSwitch(String creditCardNumber) {
        try {
            if ((creditCardNumber.length() == 16
                    || creditCardNumber.length() == 18 || creditCardNumber
                    .length() == 19)) {
    ...
    private CreditCard isVisaElectron(String creditCardNumber) {
        try {
            if (creditCardNumber.length() == 16
                    ...
                    || creditCardNumber.startsWith("4508") || creditCardNumber
                    .startsWith("4844"))) {
    ...
    private CreditCard isMastercard(String creditCardNumber) {
        try {
            if (creditCardNumber.length() == 16) {
                int firstTwoDigits = Integer.parseInt(creditCardNumber
                        .substring(0, 2));
    ...
    protected final boolean isChecksumCorrect(final String creditCardNumber) {
        try {
            ...
            for (int i = 1; i <= creditCardNumber.length(); i++) {
                int currentDigit = creditCardNumber.charAt(creditCardNumber
                        .length() - i)
                        - nulOffset;
                if ((i % 2) == 0) {
                    currentDigit *= 2;
                    currentDigit = currentDigit > 9 ? currentDigit - 9
                            : currentDigit;
                    sum += currentDigit;
                } else {
                    sum += currentDigit;
